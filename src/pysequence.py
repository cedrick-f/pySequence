...
#             print("   ", ep)
            ...
            return list(range(sum([p for _, p in self.GetReferentiel().periodes])))
...
#         return err
    ...
    def GetNbrPeriodes(self):
        return sum([p for _, p in self.GetReferentiel().periodes])
...
#             return img.Resize((w, h), (0,0)).ConvertToBitmap()
    ...
    def SelectItem(self, branche, depuisFiche = False):
        ...
        for _ in self.arbre._itemWithWindow:
...
class Projet(BaseDoc, Grammaire):
    ...
    def __init__(self, app, classe = None, intitule = "", ouverture = False):
        ...
        # code désignant le type de projet (parmi les codes de projet du référentiel)
#        print "init Projet"
...
#         print("GetPeriodeDefaut", spe)
        projet = self.GetProjetRef()  #type referentiel.Projet
#         print("   ", projet)
...
#             # Code du Referentiel.Projet correspondant au début de la période définie par self.position
#             self.code = self.GetReferentiel().getProjetEval(self.position[0]+1)
...
#                 self.initRevues()
#                 self.MiseAJourNbrRevues()
#         print("taches2", self.taches, self.calcNbrRevues(), self.getNbrRevues())        
#         # Correction de corruption
#         if prj is not None and self.calcNbrRevues() != self.getNbrRevues():
#             self.MiseAJourNbrRevues()
#         print("taches3", self.taches)
#         
...
#         print("taches3", self.taches)
        ...
        if branchesysml is not None and 'SML' in prj.attributs and prj.attributs['SML'][0] != "":
            for i, _ in enumerate(prj.attributs['SML'][2]):
                code = "SML"+str(i)
...
#        return n-1
    ...
    def SetPosition(self, pos, first = False):
        """ Change la position [début, fin] du Projet
            et gère les éventuelles revues (ajout/suppression)
        """
#        print "SetPosition", pos
#        print "  position actuelle :", self.position
#        posEpreuve = self.GetProjetRef().getPeriodeEval()

        # Code du Referentiel.Projet correcpondant au début de la periode de la position du Projet
...
#         print("   listeParties:", prjeval.listeParties)
        ...
        indicateurs = [{} for _ in range(len(self.eleves)+1)]   # 0 : tous les élèves
...
#         print("   self.compVisees", self.compVisees)
        ...
        for c in sorted(comp.competences):
#             print("   ", c)
...
#         return nomFichiers
    ...
    ######################################################################################  
    def GetNomGrilles(self, dirpath = None, win = None):
...
#        print prj.grilles
        ...
        # Par défaut = chemin du fichier .prj
        if dirpath == None:
            dirpath = os.path.dirname(doc.GetApp().fichierCourant)
        ...
        for fo, parts in prefixes.items():
            ...
            if grilles.EXT_EXCEL != None: 
                if gr[1] == 'C': # fichier "Collectif"
                    nomFichiers[fo] = os.path.join(dirpath, doc.getNomFichierDefaut(prefixe))
                else:
                ...
                continuer = True
                while continuer:
                    if grilles.EXT_EXCEL != "":
                        nomFichiers[fo] += grilles.EXT_EXCEL
                        continuer = False
                    else:
                        dialog = wx.MessageDialog(win, "Excel est installé mais semble occupé\n\n"\
                                                  "Veuillez fermer Excel ou bien vérifier qu'aucune cellule n'est en cours d'édition.\n\n"\
                                                  "Continuer ?", 
...
#         self.GetDocument().GetApp().MarquerFichierCourantModifie()
        ...
        if messageFin:
            ...
            if len(tableaux)>1:
                ...
                t += "de la grille"
            t += "\n\n"
            t += "\n".join([f for _,f in nomFichiers.values()])
            t += "\n\nterminée avec "
            if len(log) == 0:
                t += "succès !"
            else:
