...
class RTCSpellCheck(object):
    """Spell checking for use with wx.RichTextControl.
...
#                               self._spelling_style)
    ...
    def clearAll(self):
        ...
        attr.SetFlags(wx.TEXT_ATTR_FONT_UNDERLINE)
        attr.SetFontUnderlined(False)
        self.stc.SetStyleEx(rt.RichTextRange(0, self.stc.GetLength()), attr)
...
#        self.stc.SetStyling(self.stc.GetLength(), 0)
    ...
    def checkRange(self, start, end):
        ...
        attr.SetFlags(wx.TEXT_ATTR_FONT_UNDERLINE)
        attr.SetFontUnderlined(False)
        self.stc.SetStyleEx(rt.RichTextRange(start, end), attr)
#        self.stc.StartStyling(start, mask)
#        self.stc.SetStyling(count, 0)
        
        text = self.stc.GetRange(start, end) # note: returns unicode
        unicode_index = 0
        max_index = len(text)
...
#                            attr.SetOutlineLevel(1)
                            self.stc.SetStyleEx(rt.RichTextRange(last_pos, raw_count + last_pos-1) ,
                                                attr)
...
#                            self.stc.SetStyling(raw_count, mask)
    ...
    def getLikelyWordStart(self, pos):
        ...
        Replaces WordStartPosition, which doesn't handle apostrophes.
        """
        c, l = self.stc.PositionToXY(pos)
        start_of_line = self.stc.GetLineText(l)[:c]
#        linenum = self.stc.LineFromPosition(pos)
#        linestart = self.stc.PositionFromLine(linenum)
#        start_of_line = self.stc.GetTextRange(linestart, pos)
        if len(start_of_line) == 0 or start_of_line[-1].isspace():
...
#        rest_of_line = self.stc.GetTextRange(pos, lineend)
        ...
        c, l = self.stc.PositionToXY(pos)
        rest_of_line = self.stc.GetLineText(l)[c:]
        if len(rest_of_line) == 0 or rest_of_line[0].isspace():
            return pos
        try:
            word = rest_of_line.split()[0]
    ...
    def processDirtyRanges(self):
        cursor = self.stc.GetInsertionPoint ()
        ...
        if self.current_dirty_start >= 0:
            ...
            if cursor >= range_start and cursor <= range_end:
                ...
                attr.SetFlags(wx.TEXT_ATTR_FONT_UNDERLINE)
                attr.SetFontUnderlined(False)
                self.stc.SetStyleEx(rt.RichTextRange(word_start, word_end) ,
                                    attr)
